...
def _to_number(s) -> float | None:
    ...
    # Check for negative indicators - but be more careful about parentheses
    is_negative = s.startswith('-')
    # Only treat parentheses as negative if they wrap the entire number
    if s.startswith('(') and s.endswith(')') and s.count('(') == 1:
        is_negative = True
        s = s[1:-1]  # Remove outer parentheses
    elif s.startswith('-'):
        is_negative = True
        s = s[1:]  # Remove minus sign
    
    # Remove common non-numeric characters but preserve structure
    # Keep commas and dots for proper parsing
    s = re.sub(r'[^\d,.\-+]', '', s)
    s = s.replace('+', '')  # Remove any remaining plus signs
    ...
    if ',' in s and '.' in s:
        ...
        # Only remove if it's clearly a thousands separator pattern
...
def _find_section_key(text: str, section_map: dict) -> str | None:
    """Find which section a text belongs to with improved matching."""
    t = text.lower().strip()
    ...
    if 'hyperpure' in t or 'b2b' in t:
        return 'b2bSupplies'
    if 'going' in t and ('out' in t or 'dining' in t):
        return 'goingOut'
    if 'quick' in t and ('commerce' in t or 'blinkit' in t):
        return 'quickCommerce'
    if 'food' in t and 'delivery' in t:
        return 'foodDelivery'
    
    return None


# Updated section mapping with more comprehensive synonyms
ZOMATO_SECTIONS = {
    'foodDelivery': ['food delivery', 'delivery', 'food delivery segment'],
    'quickCommerce': ['quick commerce', 'blinkit', 'quick-commerce', 'qcommerce'],
    'goingOut': ['going-out', 'going out', 'dining out', 'dining', 'going out segment'],
    'b2bSupplies': ['b2b supplies', 'hyperpure', 'b2b', 'hyperpure segment']
}
...
def analyze_financials(request):
    ...
    # Safe executive summary formatting
    revenue_str = f"{revenue:,.0f}" if revenue else "N/A"
    expenses_str = f"{expenses:,.0f}" if expenses else "N/A"
    net_income_str = f"{net_income:,.0f}" if net_income is not None else "N/A"
    margin_str = metrics['Profit Margin (%)'] if isinstance(metrics['Profit Margin (%)'], str) else f"{metrics['Profit Margin (%)']}%"
    runway_str = metrics['Runway (Months)'] if isinstance(metrics['Runway (Months)'], str) else f"{metrics['Runway (Months)']} months"
    debt_equity_str = metrics['Debt-to-Equity Ratio'] if isinstance(metrics['Debt-to-Equity Ratio'], str) else str(metrics['Debt-to-Equity Ratio'])

    exec_summary = f"""
    For {company} during {period}, reported revenue was {revenue_str} with expenses of {expenses_str}.
    Net income stands at {net_income_str} ({margin_str} margin).
    Current runway is {runway_str} and Debt-to-Equity is {debt_equity_str}.
...
def parse_zomato_pdf(file_path: str) -> dict:
    """Extract structured data for Zomato annual report with enhanced parsing."""
    segments = {
        'foodDelivery': {},
        'quickCommerce': {},
        'goingOut': {},
    ...
    try:
        with pdfplumber.open(file_path) as pdf:
            kw = ['food delivery', 'quick commerce', 'going-out', 'going out', 'hyperpure', 'b2b', 'gov', 'revenue', 'yoy', 'blinkit']
            pages_idx = _relevant_pdf_pages(pdf, kw, max_pages=100)  # Increased page limit
            ...
            for i in pages_idx:
                ...
                for tbl in tables:
                    ...
                    target_map = {
                        'segment': ['segment', 'business', 'business segment', 'unit', 'category', 'business line'],
                        'gov': ['gov', 'gross order value', 'gmv', 'gross merchandise value'],
                        'revenue': ['revenue', 'sales', 'net sales', 'total revenue'],
                        'yoy': ['yoy', 'yoy growth', 'growth %', 'yo y', 'y-o-y', 'growth', 'yoy %'],
                        'orders': ['orders', 'total orders', 'order volume'],
                        'aov': ['aov', 'avg order value', 'average order value', 'order value'],
                    }
                    ...
                    for tgt, syns in target_map.items():
                        ...
                        for col in df.columns:
                            ...
                            score = _column_match_score(col_str, syns)
                            
                            # Special pattern matching for GOV and YoY
                            if tgt == 'gov' and re.search(r'\bGOV\b', col_str, re.I):
                                score = max(score, 0.95)
                            elif tgt == 'yoy' and re.search(r'\byoy\b|\bgrowth\b', col_str, re.I):
                                score = max(score, 0.85)
                            elif tgt == 'segment' and re.search(r'\bsegment\b|\bbusiness\b', col_str, re.I):
                                score = max(score, 0.9)
                        ...
                        col_map[tgt] = best_col if best_score >= 0.5 else None  # Lowered threshold

                    seg_col = col_map.get('segment')
                    if not seg_col:
                        # Try to find segment info in first column if no explicit segment column
                        if len(df.columns) > 0:
                            first_col = df.columns[0]
                            # Check if first column contains business names
                            sample_values = df[first_col].dropna().head(3)
                            if any(_find_section_key(str(val), ZOMATO_SECTIONS) for val in sample_values):
                                seg_col = first_col
                                col_map['segment'] = seg_col
                    ...
                    for idx, row in df.iterrows():
                        seg_text = str(row.get(seg_col, '')).strip()
                        ...
                        for metric, col_name in col_map.items():
                            if metric == 'segment' or not col_name:
                                continue
                            ...
                            if metric == 'yoy':
                                # Handle YoY growth specially - keep as string with formatting
                                yoy_str = str(raw_val).strip()
                                if yoy_str and yoy_str != '-':
                                    segments[section]['YoYGrowth'] = yoy_str
                            else:
                                # Convert to number
                                ...
                                if num_val is not None and num_val != 0:
                                    ...
                                    segments[section][metric_name] = num_val
    except Exception as e:
        print(f"Error in parse_zomato_pdf: {e}")
